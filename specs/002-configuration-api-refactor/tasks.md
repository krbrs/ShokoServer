...
- [X] Review the existing `ConfigurationController` routes, response types, and compatibility-sensitive behaviors.
- [X] Identify every route that will remain public, be renamed, or be kept only as a compatibility alias.
- [X] Identify every controller behavior that currently depends on `HasCustomLoad`, `HasCustomSave`, `HasCustomNewFactory`, `HasCustomValidation`, `HasCustomActions`, or `HasLiveEdit`.
...
- [X] Create a request model for configuration discovery filters so the list endpoint no longer needs a long parameter list.
- [X] Create request models for action-oriented operations where the body currently accepts generic JSON and extra query parameters.
- [X] Add any shared response models needed to keep action outcomes explicit and self-documenting.
...
- [X] Extract the controller-facing branching around load, save, new, validation, custom actions, and live edit into a focused service helper or dispatcher.
- [X] Ensure the controller can request an operation without checking the full capability matrix itself.
- [X] Preserve existing behavior for supported and unsupported configuration capabilities.
...
- [X] Replace the long list of tri-state query parameters on the configuration list endpoint with the new discovery filter model.
- [X] Keep discovery results stable and continue returning IDs and identifying metadata.
- [X] Preserve search and plugin scoping behavior while simplifying the call site.
...
- [X] Keep a dedicated metadata route for a single configuration.
- [X] Make the main configuration read route return the editable document in a clear, predictable way.
- [X] Keep the schema route aligned with the same configuration ID and document flow.
...
- [X] Update the partial-update route so it follows the same document-editing semantics as the full-update route.
...
- [X] Move validation to an explicit route with a request model that matches the public contract.
- [X] Move custom action execution to an explicit action route with a narrow, predictable shape.
- [X] Move live-edit handling to an explicit route with clear semantics for the caller.
...
- [X] Return standard success codes for successful configuration reads and writes.
