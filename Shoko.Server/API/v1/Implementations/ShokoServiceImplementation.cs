...
public partial class ShokoServiceImplementation : Controller
{
    ...
    [HttpGet("Bookmark")]
    public List<object> GetAllBookmarkedAnime()
    {
        return [];
    }

    [HttpPost("Bookmark")]
    public CL_Response<object> SaveBookmarkedAnime([FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Allow)] object contract)
    {
        return new CL_Response<object> { ErrorMessage = "No longer supported" };
    }
    ...
    public object GetBookmarkedAnime(int bookmarkedAnimeID)
    {
        return null;
    }
    ...
    public List<CL_AniDB_Anime> GetMiniCalendar(int userID, int numberOfDays)
    {
        ...
        try
        {
            ...
            foreach (var anime in animes)
            {
                ...
                animeList.Add(_legacyV1Service.GetV1Contract(anime));
            }
        }
        catch (Exception ex)
        {
    ...
    public List<CL_Recommendation> GetRecommendations(int maxResults, int userID, int recommendationType)
    {
        ...
        try
        {
            ...
            var allUserDataWithRatings = RepoFactory.AnimeSeries_User.GetByUserID(user.JMMUserID)
                ...
                .ToList();
            if (allUserDataWithRatings.Count == 0)
                return recs;
            ...
            foreach (var userData in allUserDataWithRatings)
            {
                ...
                foreach (var link in simAnime)
                {
                    ...
                    rec.Score = CalculateRecommendationScore(userData.AbsoluteUserRating.Value, link.GetApprovalPercentage(), animeRating);
                    rec.BasedOnVoteValue = userData.AbsoluteUserRating.Value;
                    rec.RecommendedApproval = link.GetApprovalPercentage();

                    // check if we have added this recommendation before
                    // this might happen where anime are recommended based on different votes
                    ...
                    if (animeLink != null)
                    {
                        rec.Recommended_AniDB_Anime = _legacyV1Service.GetV1Contract(animeLink);
                    }

                    rec.BasedOn_AniDB_Anime = _legacyV1Service.GetV1Contract(anime);
                    ...
                    rec.BasedOn_AnimeSeries = _legacyV1Service.GetV1UserContract(serBasedOn, userID);

                    dictRecs[rec.RecommendedAnimeID] = rec;
                }
            }

            // sort by the highest score and take the `maxResults` amount
            recs = dictRecs.Values.OrderByDescending(a => a.Score).Take(maxResults).ToList();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "{Ex}", ex);
    ...
    public List<CL_AniDB_Character> GetCharactersForAnime(int animeID)
    {
        ...
        try
        {
            ...
            return _legacyV1Service.GetCharactersContract(anime);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "{Ex}", ex);
